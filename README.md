...
| `[business] 00007 …` 같은 오류 | HTTP 는 200 이지만 **업무 오류**입니다. **판정은 `rsp_msg` 내용이 우선**이며 `rsp_cd` 는 API 마다 정상/오류가 갈립니다. 서버가 1차 판정하지만 전수가 아니므로 메시지를 확인하세요. 1차 기준 교체는 `NHPLUG_SUCCESS_CODES` |
